...
def get_active_firmware_line() -> Optional[str]:
    try:
        ...
        matching_lines = [line for line in all_lines if "Firmware: intel/vpu"
                          in line]

        if not matching_lines:
            print("Error: No 'intel_vpu' firmware logs found in dmesg.",
                  file=sys.stderr)
            return None
...
def main() -> int:
    ...
    if not FIRMWARE_SEARCH_DIR.is_dir():
        print(f"Error: Firmware directory not found at \
              '{FIRMWARE_SEARCH_DIR}'", file=sys.stderr)
        return 1
    ...
    print("\nTest failure: The loaded firmware does not match any version in \
          the snap files.", file=sys.stderr)
    print(f"   Loaded dmesg line contains: '{active_firmware_line}'",
          file=sys.stderr)
    return 1
