...
class TestVersionPattern(unittest.TestCase):
    def test_date_format_numeric(self):
        ...
        self.assertIsNotNone(check_firmware_version.VERSION_PATTERN.match(line))
    ...
    def test_previous_releases(self):
        ...
        for version_string in previous_releases:
            self.assertIsNotNone(check_firmware_version.VERSION_PATTERN.match(version_string))
...
class TestGetActiveFirmwareLine(unittest.TestCase):
    ...
    def test_no_match_found(self, mock_run, mock_stderr):
        ...
        self.assertIn("No 'intel_vpu' firmware logs found", mock_stderr.getvalue())

    @patch('sys.stderr', new_callable=io.StringIO)
    @patch('subprocess.run', side_effect=subprocess.CalledProcessError(1, "cmd"))
    def test_subprocess_error(self, mock_run, mock_stderr):
        """Test when 'journalctl' command fails."""
        result = check_firmware_version.get_active_firmware_line()
        self.assertIsNone(result)
        self.assertIn("Error: Could not execute", mock_stderr.getvalue())

    @patch('sys.stderr', new_callable=io.StringIO)
    @patch('subprocess.run', side_effect=FileNotFoundError("command not found"))
    def test_filenotfound_error(self, mock_run, mock_stderr):
        """Test when 'journalctl' or 'sudo' is not found."""
        result = check_firmware_version.get_active_firmware_line()
        self.assertIsNone(result)
        self.assertIn("Error: Could not execute", mock_stderr.getvalue())
...
class TestFindVersionInFile(unittest.TestCase):
    ...
    def test_success_finds_first_match(self, mock_run):
        ...
        self.assertEqual(result, "20250925*MTL_CLIENT_SILICON-NVR+NN-deployment*2485cfeafeed591eaa9a320bfae2407c1b83b29f*2485cfeafeed591eaa9a320bfae2407c1b83b29f*2485cfeafee")
        mock_run.assert_called_with(
            ['sudo', 'strings', dummy_path],
            capture_output=True, text=True, check=True, encoding='utf-8'
        )
    ...
    def test_no_match(self, mock_run):
        ...
        result = check_firmware_version.find_version_in_file(Path('/fake/fw.bin'))
        self.assertIsNone(result)
...
class TestMainFunction(unittest.TestCase):
    ...
    @patch('check_firmware_version.FIRMWARE_SEARCH_DIR', new_callable=MagicMock)
    @patch('check_firmware_version.find_version_in_file')
    @patch('check_firmware_version.get_active_firmware_line')
    def test_main_success(self, mock_get_line, mock_find_version, mock_fw_dir,
    ...
    @patch('check_firmware_version.find_version_in_file')
    @patch('check_firmware_version.get_active_firmware_line')
    def test_main_fail_no_match(self, mock_get_line, mock_find_version, mock_fw_dir,
                                mock_stdout, mock_stderr):
        active_line = "[   14.967341] intel_vpu 0000:00:0b.0: [drm] Firmware: intel/vpu/vpu_37xx_v1.bin, version: 20250925*MTL_CLIENT_SILICON-NVR+NN-deployment*2485cfeafeed591eaa9a320bfae2407c1b83b29f*2485cfeafeed591eaa9a320bfae2407c1b83b29f*2485cfeafee"
        ...
        self.assertIn("Test failure", mock_stderr.getvalue())
        self.assertIn(active_line, mock_stderr.getvalue())
    ...
    @patch('check_firmware_version.find_version_in_file')
    @patch('check_firmware_version.get_active_firmware_line')
    def test_main_fail_no_active_line(self, mock_get_line, mock_find_version, mock_fw_dir,
                                      mock_stdout, mock_stderr):
        ...
        # get_active_firmware_line failed so main shouldn't have any stdout and return 1
        self.assertEqual(return_code, 1)
        self.assertEqual(mock_stdout.getvalue(), "")
    ...
    @patch('check_firmware_version.find_version_in_file')
    @patch('check_firmware_version.get_active_firmware_line')
    def test_main_fail_no_directory(self, mock_get_line, mock_find_version, mock_fw_dir,
                                    mock_stdout, mock_stderr):
        ...
        self.assertIn("Error: Firmware directory not found", mock_stderr.getvalue())
