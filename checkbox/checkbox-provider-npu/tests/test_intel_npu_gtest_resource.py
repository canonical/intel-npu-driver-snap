...
class TestHelperFunctions(unittest.TestCase):
    ...
    def test_get_metric_streamer_allowed_states(self):
        ...
        self.assertEqual(
            intel_npu_gtest_resource.get_metric_streamer_allowed_states("MetricQuery"),
            ["supported"]
        )
        self.assertEqual(
            intel_npu_gtest_resource.get_metric_streamer_allowed_states("MetricQueryPool"),
            ["supported"]
        )
        self.assertEqual(
            intel_npu_gtest_resource.get_metric_streamer_allowed_states("Metric"),
            ["supported"]
        )

        # Other category names that shouldn't require metric streamer support
        self.assertEqual(
            intel_npu_gtest_resource.get_metric_streamer_allowed_states("Command"),
            ["supported", "unsupported"]
        )
        self.assertEqual(
            intel_npu_gtest_resource.get_metric_streamer_allowed_states("ZeInitTest"),
            ["supported", "unsupported"]
        )
    ...
    def test_is_known_failure(self):
        self.assertTrue(intel_npu_gtest_resource.is_known_failure(
        ...
        self.assertFalse(intel_npu_gtest_resource.is_known_failure(
            "Umd", "File"
        ))
...
class TestMainFunction(unittest.TestCase):
    ...
    @patch.dict('os.environ', {'HOME': '/fake/home'})
    def test_main_logic_with_filtering(self, mock_run, mock_stdout):
        ...
        expected_config_path = '/fake/home/snap/intel-npu-driver/current/basic.yaml'
        mock_run.assert_called_once_with(
            ["intel-npu-driver.npu-umd-test", "-l", "--config", expected_config_path],
        ...
        self.assertNotIn("name: executeManyLowPriorityJobsExpectHighPriorityJobCompletesFirst", output)


    @patch('subprocess.run')
    @patch.dict('os.environ', {'HOME': '/fake/home'})
    def test_main_subprocess_not_found(self, mock_run):
        # Check that the script will raise an exception if the script or the
